...
def process_gather_detection(video_path, roi=None, threshold=None):
    """处理聚集检测视频任务"""
    import cv2
    import numpy as np
    import os
    ...
    # 核心参数配置
    ROI = roi if roi is not None else [(220, 300), (700, 300), (700, 700), (200, 700)]
    聚集阈值 = threshold if threshold is not None else 5
    ...
    output_video_path = os.path.join(processed_dir, f"gather_processed_{datetime.now().strftime('%Y%m%d_%H%M%S')}.mp4")
    ...
    while True:
        ...
        # 显示ROI内人数
        frame = cv2_puttext(frame, f"ROI内人数: {roi_person_count}", (30, 50), font_scale=1, color=(255, 0, 0),
                            thickness=2)
...
if __name__ == "__main__":
    ...
    try:
        project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        model_path = os.path.join(project_root, "yolov12", "yolov12n.pt")
        model = YOLO(model_path)  # 加载模型（可替换为yolov12s.pt等其他版本）
